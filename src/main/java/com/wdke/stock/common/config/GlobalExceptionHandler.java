...
import com.wdke.stock.common.consts.KeyConstant;
import com.wdke.stock.common.enums.ResponseStatusEnum;
import com.wdke.stock.common.enums.ReturnStatusEnum;
...
public class GlobalExceptionHandler {
    ...
    public void processValidationError(HttpServletResponse response, MethodArgumentNotValidException ex) {
        ...
        try {
            ...
            String errors = this.processFieldErrorsStr(fieldErrors);
            JSONObject json = new JSONObject();
            json.put(KeyConstant.KEY_RESULT_STATUS, ResponseStatusEnum.PARRAM_ERROR.getCode());
            json.put(KeyConstant.KEY_RESULT_MESSAGE, errors);
            logger.info("processValidationError msg:{}",ex.getMessage());
            writer.write(json.toJSONString());
        } catch (IOException e) {
    ...
    public void malformedJwtException(HttpServletResponse response, RuntimeException ex) {
        ...
        try {
            writer = response.getWriter();
            JSONObject json = new JSONObject();
            json.put(KeyConstant.KEY_RESULT_STATUS, ResponseStatusEnum.INVALID_TOKEN.getCode());
            json.put(KeyConstant.KEY_RESULT_MESSAGE, "令牌错误，无权限操作");
